...
const bookController = {
  async generate(req, res) {
    try {
      ...
      const materialCompleto = await ejs.renderFile(
        path.resolve(__dirname, '..', 'views', 'material_completo.ejs'),
        { questions: questoes, posts: postagens }
      )

      const conteudoPerguntas = await ejs.renderFile(
      ...
      const materiais = [materialCompleto, conteudoPerguntas, conteudo]
  ...
  async download(req, res) {
    try {
      const material = path.resolve(
        __dirname,
        '..',
        'assets',
        'materials',
        `${req.query.material}.pdf`
      )
